...
def add_ledger_account(user_id, id_token, account_name):
    ...
    existing_accounts = get_all_ledger_accounts(user_id, id_token)
...
def get_all_ledger_accounts(user_id, id_token):
...
def get_ledger_account_by_id(user_id, id_token, account_id):
...
def get_ledger_account_by_name(user_id, id_token, account_name):
    """Retrieves a single ledger account by its name for a user."""
    accounts = get_all_ledger_accounts(user_id, id_token)
    for account in accounts:
        if account['account_name'] == account_name:
            return account
    return None

def add_entry_to_ledger_account(user_id, id_token, account_id, transaction_name, description, transaction_type, billing_number, amount, payment_method):
...
def delete_entry_from_ledger_account(user_id, id_token, account_id, entry_index):
...
def mark_ledger_entry_paid(user_id, id_token, account_id, entry_index, paid_status):
...
def delete_ledger_account(user_id, id_token, account_id):
